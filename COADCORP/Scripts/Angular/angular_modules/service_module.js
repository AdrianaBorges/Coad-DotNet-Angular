...
serviceModule.factory('formHandlerService', ['$http', '$parse', 'conversionService', '$timeout',
    function ($http, $parse, conversionService, $timeout) {
					    ...
					    if (showLoader === true)
					        ...
					        if (typeof result['success'] == 'undefined' ||  (result['success'] && result['success'] === true)) {
					            ...
					            if (params.fail && typeof params.fail == "function") {
                                    params.fail(result, status, config)
					            }
					        }
...
serviceModule.factory("mask", ['maskDinheiro', function (maskDinheiro) {
...
serviceModule.factory("clearMask", ['clearMaskDinheiro', function (clearMaskDinheiro) {
...
serviceModule.factory("maskDinheiro", ['clearMask', function (clearMask) {
...
serviceModule.factory("cepService", ['$http', function ($http) {
...
serviceModule.factory("scopeBindService", ['$parse', function ($parse) {
...
serviceModule.factory("includeService", ['$injector', function ($injector) {
...
serviceModule.filter("as", ['$parse', function ($parse) {
...
serviceModule.factory("MathService", function () {
    ...
    return {
        ...
        ProcessarDesconto: function (preco, porcentagem, parcelas) {
            ...
            total = this.truncarDecimal(total, 2);

            var valorParcelas = (total / parcelas);
            valorParcelas = this.truncarDecimal(valorParcelas, 2);
        ...
        CalcularTotal: function (valor, numParcelas) {
            ...
            return this.truncarDecimal((numParcelas * valor), 2);
        },

        truncarDecimal: function (num, fixed) {

            var valueFixed = Number(Number(num).toFixed(fixed));
            num = Number(num);
...
serviceModule.factory("MoneyService", ['mask', 'clearMask', function (mask, clearMask) {
...
serviceModule.factory('UploadAjax', ['$http', function ($http) {

    return {

        upload: function (url, file, otherData) {
...
serviceModule.factory('AuthService', ['formHandlerService', '$interval',
    function (formHandlerService, $interval) {

    return {

        listIntervals : [],
        checarPermissao: function (nomeMetodo, nomePermissao, $scope) {
           
            var interval = $interval(function () {

                var url = Util.getUrl('/login/ValidarPermissao');
                formHandlerService.read($scope, {
                    url: url,
                    data: { methodName: nomeMetodo },
                    showAjaxLoader: true,
                    targetObjectName: nomePermissao,
                    responseModelName: 'possuiPermissao',
                    success: function () {
            ...
            this.listIntervals.push({ key: nomeMetodo, value: interval });
        }
    }
...
serviceModule.factory('FilterService', function () {

    return {

        adicionarGrupoDeFiltro: function (filtros, grupo, chave) {

            this.adicionarDadoDeFiltro(filtros, grupo, chave, "grupo", "grupo");
        },
        
        adicionarDadoCombo: function (filtros, lstCombo, chave)
        {
            var acaoProcessamento = function (filtro) {

                if (filtro.valueName && filtro.labelName) {

                    angular.forEach(lstCombo, function (value, index) {

                        value.value = value[filtro.valueName];
                        value.label = value[filtro.labelName];
                    });
                }
            };
            this.adicionarDadoDeFiltro(filtros, lstCombo, chave, "select", "listCombo", acaoProcessamento);
        },

        adicionarDadoDeFiltro: function (filtros, valor, chave, tipoRequerido, nomeObjectoAInsetir, acaoProcessamento) {
            
            if (filtros && valor && chave) {

                angular.forEach(filtros, function (grupoFiltro, index) {

                    angular.forEach(grupoFiltro.filtros, function (filtro, index1) {

                        if (filtro.chave == chave && filtro.tipo == tipoRequerido) {
                            if (acaoProcessamento && typeof acaoProcessamento == 'function') {

                                acaoProcessamento(filtro);
                            }

                            filtro[nomeObjectoAInsetir] = valor;
                        }
                    });
                });
            }
        },
        buscarFiltrosPreenchidos : function ($scope, nomeModelDiretiva) {

            $scope.list = [];

            angular.forEach($scope[nomeModelDiretiva], function (grupoFiltro, index) {
                var nomeGrupo = null;
                ...
                angular.forEach(grupoFiltro.filtros, function (filtro, index1) {
                    ...
                    if (filtro.tipo == 'grupo') {
                        ...
                        angular.forEach(filtro.grupo, function (grp, index2) {

                            if ($scope.model[grp.chave] && $scope.model[grp.chave] == grp.valor) {
                                grp.tipo = "grupo";

                                var label = null;
                                var valor = grp.label;

                                if (filtro.exibirNomeToken == true) {
                                    
                                    label = (filtro.nomeNoToken) ? filtro.nomeNoToken : grupoFiltro.nomeGrupo;
                                }
                                grp.token = {
                    ...
                    if (filtro.tipo == "select") {
                        ...
                        if ($scope.model[filtro.chave]) {

                            angular.forEach(filtro.listCombo, function (value, index) {

                                var id = $scope.model[filtro.chave];
                                if (id == value[filtro.valueName]) {

                                    var label = value[filtro.labelName];
                                    filtro.token = {
                            ...
                            $scope.list.push(filtro);
                        }
                    }
                    else
                    if (filtro.tipo == "toogle") {
                        if ($scope.model[filtro.chave]) {
                        ...
                        if ($scope.model[filtro.chave]) {
                            ...
                            var label = null;
                            var valor = (filtro.tipo == 'data' || filtro.tipo == 'autocomplete') ? $scope.model[filtro.chave + 'STR'] : $scope.model[filtro.chave];

                            if(!filtro.usarLabelToken || filtro.usarLabelToken == true){
...
serviceModule.factory('UenService', ['formHandlerService', function (formHandlerService) {

        return {
            scopes: [],
            observarUenAuterada: function (scope, campo) {

                this.scopes.push({ escopo: scope, campo: campo });
            },
            notificarUenAuterada: function (uen) {

                if (this.scopes != null) {

                    angular.forEach(this.scopes, function (value, index) {

                        value.escopo[value.campo] = uen;
                    });
                }
            },
            trocarUen: function ($scope, uenID) {
                var instance = this;
                ...
                formHandlerService.submit($scope, {
                    ...
                    success: function (resp, status, config, message, validationMessage) {
                        ...
                        if (resp.success) {
                            ...
                            var scopeUen = angular.element("#div-uen").scope();
                            scopeUen.uen = $scope.uenAtual;
                            instance.notificarUenAuterada($scope.uenAtual);                            
                        }
