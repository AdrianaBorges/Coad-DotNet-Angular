...
{
    public class ClienteAntController : Controller
    {
        ...
        [Autorizar(IsAjax = true)]
        public ActionResult BuscarListaApuracaoConsulta(string _assinatura, string _contrato)
        {
            ...
            {
                if (_contrato == null)
                    _contrato = new ContratoSRV().BuscarUltimoContrato(_assinatura);
        ...
        [Autorizar(IsAjax = true)]
        public ActionResult MostrarNossoNumero(string _idTitulo)
        {
            JSONResponse result = new JSONResponse();
            try
            {
                if (_idTitulo.Length == 0)
                    throw new Exception("Numero do título não informado. Verifique !!");

                var _nossonumero = new BoletoSRV().GerarNossoNumero("237", _idTitulo, true, true);

                result.success = true;
                result.Add("nossonumero", _nossonumero);
