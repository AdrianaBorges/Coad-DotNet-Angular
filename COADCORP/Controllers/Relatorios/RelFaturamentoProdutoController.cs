...
{
    public class RelFaturamentoProdutoController : Controller
    {
        ...
        [Autorizar(IsAjax = true)]
        public ActionResult BuscarFaturamentoProduto(DateTime _dtini
                                                    ,DateTime _dtfim
                                                    ,int? _emp_id 
                                                    ,int? _grupo_id
                                                    ,bool _tipodata)
        {
            ...
            try
            {
                var _lstFaturamentoProduto = new ContratoSRV().BuscarFaturamentoProduto(_dtini, _dtfim, _emp_id, _grupo_id, _tipodata);
                decimal? _total = 0;

                foreach (var item in _lstFaturamentoProduto)
                {
                    _total += item.VALOR_TOTAL;
                }


                response.Add("lstFaturamentoProduto", _lstFaturamentoProduto);
