...
directiveModule.directive("appPopover", function ($compile) {
...
directiveModule.directive("appListPopover", function ($compile) {
...
directiveModule.directive("appInitEditor", function ($parse) {
...
directiveModule.directive("appEditor", function ($parse) {
    ...
    return {
        ...
        link: function (scope, element, attr, ctrl) {
            ...
            scope.openEditor = function () { // editorCallerName é o id do editor que está chamando a modal
                ...
                func(scope.appEditor, attr.appEditor);
            }
        },
        template:
            '<div>' +
            '<div class="edit-modal pull-right" ng-click="openEditor()"><span class="btn-clipboard">Editar</span></div>' +
             '<figure class="highlight">' +
                    '<pre>' +
                    '<div ng-bind-html="appEditor" style="min-height: 70px;  padding-top: 26px"></div>' +
                    '</pre>' +
             '</figure>' +
             '</div>'
    };
    ...
    return {
        ...
        controller: function ($scope) {
            ...
            $scope.selecionar = function (valor) {
                ...
                if (valor) {
                    if (valor[$scope.listValue]) {

                        $scope.itemVar = angular.copy(valor);
                        $scope.appSelect2 = valor[$scope.listValue];
                    }
                    if (valor[$scope.listLabel]) {

                        $scope.ngModel = valor[$scope.listLabel];
                    }
                    else {
                        $scope.ngModel = valor;
                    }
...
directiveModule.directive("appFocus", function ($compile) {
    ...
    return {
        ...
        link: function (scope,element, attrs, transcludeFn) {
            ...
            scope.$watch(attrs.appFocus, function (value, old) {
